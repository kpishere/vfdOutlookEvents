...
void grcSslInit(Ssl::Session& session, HttpRequest& request) {
    ...
    static const uint8_t grcSha1Fingerprint_login[] PROGMEM = {0x54, 0x52, 0x80, 0x88, 0x47, 0xfe, 0x67, 0x4a, 0xba, 0x8c, 0xb6, 0x45, 0x7b, 0xd3, 0x37, 0x62, 0x7c, 0x2c, 0x56, 0xa0};
    static const uint8_t grcPublicKeyFingerprint_login[] PROGMEM = {62, 0x91, 0xaf, 0x46, 0xc3, 0x69, 0x17, 0x6a, 0x6e, 0x17, 0x6e, 0xa4, 0x6d, 0x9e, 0x2f, 0xdd, 0x57, 0x56, 0x3a, 0xcd, 0x4b, 0x0c, 0x8d, 0xb9, 0x79, 0x0f, 0x1d, 0xfe, 0x9b, 0xad, 0x86, 0x0d};

    static const uint8_t grcSha1Fingerprint_svc[] PROGMEM = {0xA2, 0x5F, 0x6E, 0xFE, 0xFD, 0x54, 0xAF, 0xF4, 0x49, 0x16, 0xEC, 0xDB, 0xD9, 0x8C, 0xE8, 0x19, 0xBD, 0xFB, 0x64, 0x54};
    static const uint8_t grcPublicKeyFingerprint_svc[] PROGMEM = {0x73, 0x0B, 0x3A, 0x91, 0xF7, 0x82, 0xDE, 0x81, 0x7C, 0x7E, 0x8D, 0xB9, 0xCA, 0x84, 0x6D, 0xC9, 0xBD, 0xE1, 0x41, 0xA6, 0x62, 0xE6, 0x06, 0x12, 0xCC, 0xF3, 0x6E, 0x14, 0xA9, 0x93, 0xAF, 0x5B};

    // Microsoft IT TLS CA 4
    if( request.uri.Host == String("login.microsoftonline.com") ) {
        fingerprints.setSha1_P(grcSha1Fingerprint_login, sizeof(grcSha1Fingerprint_login));
        fingerprints.setSha256_P(grcPublicKeyFingerprint_login, sizeof(grcPublicKeyFingerprint_login));
    } else {
        if( request.uri.Host == String("graph.microsoft.com") ) {
            fingerprints.setSha1_P(grcSha1Fingerprint_svc, sizeof(grcSha1Fingerprint_svc));
            fingerprints.setSha256_P(grcPublicKeyFingerprint_svc, sizeof(grcPublicKeyFingerprint_svc));
        } else {
            Serial.printf("No trust for %s",(const char*)request.uri.Host.c_str());
        }
    }
